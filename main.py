...
def all_rooms():
    dbs = db_session.create_session()
    if current_user.is_authenticated:
        res = dbs.query(Room).filter((Room.team_leader.like(current_user.id) | Room.collaborators.like(f'%{current_user.id}%')))
        rooms = []
        for el in res:
            title = el.title
            ...
            about = el.about
            # tasks = el.tasks
            rooms.append([title, about, team_leader, collaborators, el.id])
        return render_template('allrooms.html', rooms=rooms)
    else:
        pass

@app.route('/room/<int:room_id>')
def open_rooms(room_id):
    dbs = db_session.create_session()
    if current_user.is_authenticated:
        res = dbs.query(Room).filter((Room.id == room_id))
        taskss = []
        for el in res:
            title = el.title
            tasks = el.tasks
            taskss.append([title, tasks, el.id])
        return render_template('open_room.html', rooms=taskss)
    else:
        pass
