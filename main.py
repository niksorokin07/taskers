...
class JobForm(FlaskForm):
    email = SelectField('Почта тим лида', choices=[], validators=[DataRequired()])
    ...
    collaborators = SelectMultipleField('Участники', choices=[])
    ...
    end_date = DateTimeField('Дата окончания', format='%Y-%m-%d %H:%M:%S',
                             default=datetime.datetime(year=2023, month=1, day=1, hour=1, minute=1, second=1))
    hazard_level = IntegerField('Уровень опасности', default=0)
...
class RoomForm(FlaskForm):
    ...
    about = StringField('Описание работы', validators=[DataRequired()])
    team_leader = SelectField('Id тим лида', choices=[], validators=[DataRequired()])
